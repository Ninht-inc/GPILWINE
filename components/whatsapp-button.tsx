...
/** Floating WhatsApp contact button. Hidden until a number is set in /admin/settings. */
export function WhatsAppButton() {
  const { whatsapp_number, whatsapp_message } = useSiteSettings()

  const digits = (whatsapp_number || '').replace(/[^\d]/g, '')
  if (!digits) return null

  const href = `https://wa.me/${digits}${whatsapp_message ? `?text=${encodeURIComponent(whatsapp_message)}` : ''}`
